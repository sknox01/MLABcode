...
```{r echo=FALSE}
# Comments/Notes
# 1) Make variables generalizable for different variable names - USE AMERIFLUX NAMES HERE (make sure to update plots below) - make scatterplot generalizable
...
# 4) Remove duplicate variables (e.g., "WIND_VELOCITY_EC" = "wind_speed" -> remove "WIND_VELOCITY_EC";
#air_temperature = AIR_TEMP_2M -> could remove air_temperature)
...
```{r}
p <- scatter_plot_temp_QCQA(data,data$AIR_TEMP_2M,data$sonic_temperature-273.15,data$air_t_mean-273.15,c("sonic_temperature","air_t_mean"),"HMP Air temperature (°C)","Other Air Temperature (°C)")
toWebGL(ggplotly(p))
```
...
```{r}
scatter_plot_QCQA(data,data$RH_2M,data$RH,data$year,"HMP RH","LI-7200 RH") # 
```
