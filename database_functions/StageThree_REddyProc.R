...
StageThree_REddyProc <- function(ini_file_name,ini_path) {
  ...
  for (j in 1:length(yrs)) {
    ...
    data <- load.export.data(basepath,yrs[j],site,level_in,vars,tv_input,export)
    ...
    data_REddyProc[is.na(data_REddyProc)]<--9999
    
    #Adding the units row
    data_REddyProc <- rbind(UNITS,data_REddyProc)
    
    #Saving the file:
    write.table(data_REddyProc, file = paste(basepath,'/',yrs[j],'/',site,'/REddyProc_RF/data_EP','.txt',sep=""),row.names=FALSE,sep='\t') 
    ...
    Dir <- paste(basepath,"/",as.character(yrs[j]),"/",site,"/",level_REddyProc, sep = "")
    EddyData <- fLoadTXTIntoDataframe(file_name,Dir) 
    ...
    for (i in 1:length(var_names)) {
      writeBin(as.numeric(FilledEddyData[,i]), var_names[i], size = 4)
    }
    ...
    for (i in 1:length(vars_names_third_stage)) {
      writeBin(as.numeric(data_third_stage[,i]), vars_names_third_stage[i], size = 4)
    }
    ...
    if (fill_RF_FCH4 == 1) {
      ...
      data_RF <- load.export.data(basepath,yrs[j],site,level_RF_FCH4,predictors_FCH4,tv_input,export)
      
      # Apply gap-filling function
      datetime <- data_RF$datetime
      gap_filled_FCH4 <- RF_gf(data_RF,predictors_FCH4[1],predictors_FCH4,plot_RF_results,datetime,yrs[j],yrs)
      ...
      for (i in 1:length(var_names)) {
        writeBin(as.numeric(gap_filled_FCH4[,i]), var_names[i], size = 4)
      }
